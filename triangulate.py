...
def order_vertices(polygon):
    """Given a list of vertices in clockwise order, creates a list of the
       ...
       Returns the ordered vertice index list.
       """
    left = right = None
    ordered_indexes = []
    index = 0
...
def reflex(polygon, curr_index, stack, top_index, bottom_index):
    if len(stack) >= 2:
        """Determines if the current index from the polygon list is a part of
           the reflex chain. Returns True or False accordingly. """
...
def chain_side(index, top_index, bottom_index):
    """Given the an index, and the indices of the coordinate with the highest
       and lowest y-values in the polygon list, determine what side of the chain
       the index is on.
    """
...
def leftmost(polygon):
    """given a list of coordinates, find the minimum x_coordinate and the index
       of this point"""
    minx = polygon[0][0]
    miny = polygon[0][1]
    minindex = 0
...
def findz(polygon, u, w, v):
    ...
    for index in range(len(polygon) - 1):
        ...
        """checks if there in an intersection between any line segment
           in the polygon and (u,w). Also checks if an endpoint from any line
           segment is on the line uw.
        """
        if intersection_found((polygon[index], polygon[index + 1]), (u, w)) or \
           orient(u, w, polygon[index])  == 0 or \
           orient(u, w, polygon[index + 1])  == 0:
           intersection = True
           break
    ...
    for index in range(len(polygon)):
        ...
        # or on the edge of the triange
        if orient(v, w, vertex) != 1 and orient(w, u, vertex) != 1 and \
           orient(u, v, vertex) != 1 and v != vertex and w != vertex \
           and u != vertex:
            ...
            """determining a new point that is above the vertex.
            The line created by the vertex and the new point has the same slope
            as uw so that it is as if we are sweeping a line parallel to uw"""
            if slope != None:
                # b from y = mx + b
                ...
                newpointx = x_coordinate

            """if the current min is to the right of the line segment defined by
...
def generate_monotone_polygon(num_vertices: int) -> List[Tuple[int,int]]:
    ...
    Describe how you generate the points so that the polygon is y-monotone: I
    ensure that the polygons I generate are y monotone by making sure there are
    no split or merge vertics. I make sure to handle the 3 cases for
    monotone_triangulate by making reflex chains, having vertices on differnet
    chain sides, and breaking reflex chains.
    """
    if num_vertices < 3:
        return []
    ...
    """ adds vertices to the polygon until the length of the polygon is the
        ...
        3 cases in monotone_triangulate"""
...
def monotone_triangulate(polygon: List[Tuple[int,int]]) -> List[Tuple[int,int]]:
    ...
    diagonals = []
    ordered_vertices_indexes = order_vertices(polygon)
    top_index = ordered_vertices_indexes[0]
    bottom_index = ordered_vertices_indexes[-1]
    ...
    for index in range(2, len(polygon) - 1):
        ...
        if chain_side(stack[-1], top_index, bottom_index) == \
           chain_side(vj_index, top_index, bottom_index):
            ...
            not reflex(polygon, vj_index, stack, top_index, bottom_index):
...
def test() -> None:
    ...
    #polygon = [(3,6), (6,5), (0, 2.5), (4,0), (5,2), (3,3), (8,4), (7,-2), (-4,1)]
    #polygon = [(-1,-3), (-2, -2), (0,0), (2,-3), (5, -5), (-2,-15), (-6,-6)]
    #polygon = [(-2,-5),(0,0),(1,-2),(2.5,-3),(4,-3.5),(5,-3.7)]

    polygon = [(0,0),(0,-6),(-3,-4),(-4,-3.5),(-2.5,-3),(-1,-2)]
    print(polygon)
    diagonal = monotone_triangulate(polygon)
    print(diagonal)
    print("Brute Force:")
    error_count = 0
    for index in range(3, 200):
        polygon = generate_monotone_polygon(index)
        diagonal = brute_force_triangulate(polygon)
        if not is_triangulation(polygon, diagonal):
            print("error in brute_force_triangulate")
            error_count += 1
    print("error count")
    print(error_count)

    print("monotone_triangulate:")
    error_count = 0
    for index in range(3, 200):
        polygon = generate_monotone_polygon(index)
        diagonal = monotone_triangulate(polygon)
